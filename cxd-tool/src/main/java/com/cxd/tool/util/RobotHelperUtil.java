...
public class RobotHelperUtil {
	 ...
	 * 钉钉群设置 webhook, 支持重置
	 */
	private static final String ACCESS_TOKEN = "https://oapi.dingtalk.com/robot/send?access_token=************";
	/**
	 * 消息类型
	 */
	 ...
	 * @date: 2019/7/6
	 */
	public static void sendMessageByText(String msg) {
		...
		try {
			...
			log.error("[ApiException]: 消息发送异常捕获{}", e.getMessage());
		}
	}
	...
	public static OapiRobotSendResponse sendMessageByLink(String title, String text, String messageUrl, String picUrl) {
		if (StringUtils.isBlank(title, text, messageUrl)) {
			return null;
		}
		//参数	参数类型	必须	说明
	...
	public static OapiRobotSendResponse sendMessageByMarkdown(String title, String markdownText, List<String> mobileList, boolean isAtAll) {
		if (StringUtils.isBlank(title, markdownText)) {
			return null;
		}
		//参数	类型	必选	说明
	...
	public static OapiRobotSendResponse sendMessageByActionCardMulti(String title, String markdownText, List<OapiRobotSendRequest.Btns> btns, boolean btnOrientation, boolean hideAvatar) {
		if (StringUtils.isBlank(title, markdownText) || CollectionUtils.isEmpty(btns)) {
			return null;
		}
		//参数	类型	必选	说明
		//msgtype	string	true	此消息类型为固定actionCard
	...
	public static void main(String args[]) {
		sendMessageByMarkdown("ERROR", "###1982791749182\n > 你家的粉丝的积分圣诞节分厘卡是你家的粉丝的积分圣诞节分厘卡是你", null, false);
		//sendMessageByText("error>1982791749182>你家的粉丝的积分圣诞节分厘卡是你家的粉丝的积分圣诞节分厘卡是你家的粉丝的积分圣诞节分厘卡是你家的粉丝的积分圣诞节分厘卡是");
	}
}